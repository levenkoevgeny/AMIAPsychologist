...
class Adaptation(models.Model):
    ...
    date_added = models.DateTimeField(auto_now=True, verbose_name="Дата заполнения")
    ...
    average_score = models.FloatField(verbose_name="26. Средний балл по итогам первой сессии")
    ...
    tension = models.ManyToManyField(Tension, verbose_name="51. Что вызывает у Вас наибольшее напряжение, неудовлетворенность", blank=True)
    tension_other = models.TextField(verbose_name="51_1. Что вызывает у Вас наибольшее напряжение, неудовлетворенность (иное)", blank=True, null=True)
    ...
    need_a_conversation_theme = models.TextField(verbose_name="54.1 Тема беседы с психологом", blank=True, null=True)
    ...
    def get_tensions(self):
        result = ''
        for tension in Tension.objects.all():
            there_is = False
            for item in self.tension.all():
                if tension.id == item.id:
                    there_is = True
            if there_is:
                result = result + '1'
            else:
